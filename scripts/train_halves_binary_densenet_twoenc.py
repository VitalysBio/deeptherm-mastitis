...
from src.models.halves_binary_densenet_twoenc import (
    DenseNet121HalvesBinaryTwoEnc,
    freeze_backbone,
    unfreeze_last_block,
    trainable_params,
)
...
def evaluate(model, loader, device):
    ...
    pred_df = pd.DataFrame({"y_true": y_true, "prob": probs})
    return out, pred_df
...
def run_phase(
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        if val_metrics["f1"] > best_f1 + 1e-6:
            ...
            val_preds.to_csv(run_dir / f"best_{phase_name}_val_predictions.csv", index=False)
        else:
            no_improve += 1
...
def train_one_fold(project_root, csv_path, fold, image_view, batch_size, seed):
    set_seed(seed)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    train_ds = TIDSHalvesBinaryDataset(
        DatasetConfig(
    ...
    val_ds = TIDSHalvesBinaryDataset(
        DatasetConfig(
            project_root=project_root,
            csv_path=csv_path,
            image_view=image_view,
    ...
    model = DenseNet121HalvesBinaryTwoEnc(pretrained=True).to(device)
    criterion = nn.BCEWithLogitsLoss()

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    run_name = f"halves_binary_twoenc_densenet121_{image_view}_fold{fold}_{timestamp}"
    run_dir = project_root / "reports" / "runs_halves_binary_twoenc" / run_name
    ...
    hist1 = run_phase(
        ...
        criterion, optimizer1, None,
        max_epochs=8, patience=3,
        run_dir=run_dir, phase_name="head"
    )
    ...
    hist2 = run_phase(
        ...
        run_dir=run_dir, phase_name="finetune"
    )
