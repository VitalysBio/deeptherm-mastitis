...
from src.models.halves_multitask_densenet_twoenc import (
    DenseNet121HalvesMultitaskTwoEnc,
    freeze_backbone,
    unfreeze_last_block,
    trainable_params,
)
...
def train_one_fold(project_root, csv_path, fold, image_view, batch_size, seed, swap_lr=False):
    ...
    model = DenseNet121HalvesMultitaskTwoEnc(pretrained=True).to(device)

    criterion_bin = nn.BCEWithLogitsLoss()
    criterion_cls = nn.CrossEntropyLoss()

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    run_name = f"halves_twoenc_densenet121_{image_view}_fold{fold}_{timestamp}"
    run_dir = project_root / "reports" / "runs_halves_twoenc" / run_name
