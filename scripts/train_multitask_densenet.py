...
from src.models.multitask_densenet import (
...
def train_one_fold(
    ...
    model = DenseNet121Multitask(pretrained=True).to(device)

    criterion_bin = nn.BCEWithLogitsLoss()
    criterion_cls = nn.CrossEntropyLoss()

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    run_name = f"multitask_densenet121_{image_view}_fold{fold}_{timestamp}"
    run_dir = project_root / "reports" / "runs_multitask" / run_name
