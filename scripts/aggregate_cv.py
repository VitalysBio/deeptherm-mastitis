...
def print_mean_sd(df: pd.DataFrame, title: str):
    print(f"\n{title}")
    for m in ["f1", "precision", "recall", "pr_auc", "roc_auc", "mcc", "acc"]:
...
def main():
    ...
    # Quick table
    cols_show = [c for c in ["phase","fold","best_epoch","f1","precision","recall","pr_auc","roc_auc","mcc","acc","lr"] if c in out_df.columns]
