...
class PadToSquare:
    """
    Symmetric padding to a square canvas.
    """
    ...
    def __call__(self, img: Image.Image) -> Image.Image:
        ...
        if pad_w < 0 or pad_h < 0:
            raise ValueError(f"Image larger than target square: {(w, h)} vs {self.size}")
...
def get_transforms_halves(mode: str, image_size: int = 224):
    """
    Preprocessing for split-udder model.

    IMPORTANT:
    We do NOT use horizontal flip here because flipping would swap left/right labels.
    """
